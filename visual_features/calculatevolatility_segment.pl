...
#Output file
my $volatilityResults_segment = 'C:\\Users\\SIGHT\\Documents\\LineGraph\\volatility\\results\\volatilityResults_segments.txt';
$filename = basename($suggestionXMLfile);

# subroutine to get the points from the segments in LxBNresultPar.txt file
sub getPoints
{
...
$test		= pop(@Xpoints);
&calculateResult(@Ypoints);				# main call

# this sub receives all the sample points: Ypoints
sub calculateResult
{
	local $xLargest 			= 0.5813; #those two values need to be in an external file, so we can learn new ones
	local $yBiggest				= 0.9470;
	...
	for ($j = 0; $j < (@allintervals."" - 1); ++$j){
		...
		$new_sizeweight 	= ($sizeweight / $yBiggest);
		$new_frequencyweight= ($frequencyweight / $xLargest); 
		$result 			= ($new_sizeweight + $new_frequencyweight) / 2;
		open FILE2,">>$volatilityResults_segment";
		print FILE2 "$filename $beginning $end $j $result $new_frequencyweight $new_sizeweight\n";
		close FILE2;
